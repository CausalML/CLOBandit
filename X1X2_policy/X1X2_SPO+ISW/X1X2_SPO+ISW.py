...
def Double_robust(X_train_False,X_train, Y_transpose_Z_observed_train, Y_transpose_Z_expected_train,Z_train,Z_train_idxs_1,
    ...
    Sigma_ideal_matrix_3=np.zeros((d,d))
    for i in range(50):
        if i <=24:
         temp_matrix=3*np.outer(Z[:,i],Z[:,i])
         Sigma_ideal_matrix_3=Sigma_ideal_matrix_3+temp_matrix
        else:
         temp_matrix=np.outer(Z[:,i],Z[:,i])
         Sigma_ideal_matrix_3=Sigma_ideal_matrix_3+temp_matrix
    Sigma_ideal_matrix_3=Sigma_ideal_matrix_3/100
    
    Sigma_ideal_matrix_4=np.zeros((d,d))
    for i in range(50):
        if i <=24:
         temp_matrix=np.outer(Z[:,i],Z[:,i])
         Sigma_ideal_matrix_4=Sigma_ideal_matrix_4+temp_matrix
        else:
         temp_matrix=3*np.outer(Z[:,i],Z[:,i])
         Sigma_ideal_matrix_4=Sigma_ideal_matrix_4+temp_matrix
    Sigma_ideal_matrix_4=Sigma_ideal_matrix_4/100
    ...
    Sigma_matrix_3_ideal_inverse = np.linalg.pinv(Sigma_ideal_matrix_3)    
    Sigma_matrix_4_ideal_inverse = np.linalg.pinv(Sigma_ideal_matrix_4)  
    ...
    Ideal_U, Ideal_Sigma,Ideal_V = np.linalg.svd(Sigma_ideal_matrix_3)
    for i in range(len(Ideal_Sigma)):
      if Ideal_Sigma[i]<0.00000000001:
        Ideal_Sigma[i]=0
      if Ideal_Sigma[i]>0.00000000001 and Ideal_Sigma[i]<1:
        Ideal_Sigma[i]=1
    Modeified_ideal_Sigma_matrix_3=np.dot(np.dot(Ideal_U,np.diag(Ideal_Sigma)),Ideal_V)
    Modeified_ideal_Sigma_matrix_3_inverse=np.linalg.pinv(Modeified_ideal_Sigma_matrix_3)

    Ideal_U, Ideal_Sigma,Ideal_V = np.linalg.svd(Sigma_ideal_matrix_4)
    for i in range(len(Ideal_Sigma)):
      if Ideal_Sigma[i]<0.00000000001:
        Ideal_Sigma[i]=0
      if Ideal_Sigma[i]>0.00000000001 and Ideal_Sigma[i]<1:
        Ideal_Sigma[i]=1
    Modeified_ideal_Sigma_matrix_4=np.dot(np.dot(Ideal_U,np.diag(Ideal_Sigma)),Ideal_V)
    Modeified_ideal_Sigma_matrix_4_inverse=np.linalg.pinv(Modeified_ideal_Sigma_matrix_4)
    
    Sigma_sample_matrix_1=np.zeros((d,d))
    Sigma_sample_matrix_2=np.zeros((d,d))
    Sigma_sample_matrix_3=np.zeros((d,d))
    Sigma_sample_matrix_4=np.zeros((d,d))
    num_1=0
    num_2=0
    num_3=0
    num_4=0
    for i in range(m):
       ...
       if X[1,i]<=0 and X[2,i]>0:
           temp_matrix=np.outer(Z[:,i],Z[:,i])
           Sigma_sample_matrix_3=Sigma_sample_matrix_3+temp_matrix
           num_3=num_3+1
       if X[1,i]<=0 and X[2,i]<=0:
           temp_matrix=np.outer(Z[:,i],Z[:,i])
           Sigma_sample_matrix_4=Sigma_sample_matrix_4+temp_matrix
           num_4=num_4+1
    ...
    Sigma_sample_matrix_3=Sigma_sample_matrix_3/num_3
    Sigma_matrix_3_sample_inverse = np.linalg.pinv(Sigma_sample_matrix_3)    
    Sigma_sample_matrix_4=Sigma_sample_matrix_4/num_4
    Sigma_matrix_4_sample_inverse = np.linalg.pinv(Sigma_sample_matrix_4)
    ...
    sample_U, sample_Sigma,sample_V = np.linalg.svd(Sigma_sample_matrix_3)
    for i in range(len(sample_Sigma)):
      if sample_Sigma[i]<0.00000000001:
        sample_Sigma[i]=0
      if sample_Sigma[i]>0.00000000001 and sample_Sigma[i]<1:
        sample_Sigma[i]=1
    Modeified_sample_Sigma_matrix_3=np.dot(np.dot(sample_U,np.diag(sample_Sigma)),sample_V)
    Modeified_sample_Sigma_matrix_3_inverse=np.linalg.pinv(Modeified_sample_Sigma_matrix_3)

    sample_U, sample_Sigma,sample_V = np.linalg.svd(Sigma_sample_matrix_4)
    for i in range(len(sample_Sigma)):
      if sample_Sigma[i]<0.00000000001:
        sample_Sigma[i]=0
      if sample_Sigma[i]>0.00000000001 and sample_Sigma[i]<1:
        sample_Sigma[i]=1
    Modeified_sample_Sigma_matrix_4=np.dot(np.dot(sample_U,np.diag(sample_Sigma)),sample_V)
    Modeified_sample_Sigma_matrix_4_inverse=np.linalg.pinv(Modeified_sample_Sigma_matrix_4)
    ...
    Sigma_matrix_3_adjustment_ideal_lambda_1=Sigma_ideal_matrix_3+np.eye(d,d)
    Sigma_matrix_4_adjustment_ideal_lambda_1=Sigma_ideal_matrix_4+np.eye(d,d)
    ...
    Sigma_matrix_3_adjustment_ideal_lambda_inverse=np.linalg.inv(Sigma_matrix_3_adjustment_ideal_lambda_1)
    Sigma_matrix_4_adjustment_ideal_lambda_inverse=np.linalg.inv(Sigma_matrix_4_adjustment_ideal_lambda_1)
    ...
    Sigma_matrix_3_adjustment_sample_lambda_1=Sigma_sample_matrix_3+np.eye(d,d)
    Sigma_matrix_4_adjustment_sample_lambda_1=Sigma_sample_matrix_4+np.eye(d,d)
    ...
    Sigma_matrix_3_adjustment_sample_lambda_inverse=np.linalg.inv(Sigma_matrix_3_adjustment_sample_lambda_1)
    Sigma_matrix_4_adjustment_sample_lambda_inverse=np.linalg.inv(Sigma_matrix_4_adjustment_sample_lambda_1)
    ...
    for i in range(m): 
      ...
      if X[1,i]<=0 and X[2,i]>0:
        ...
        #d_2.append(np.dot(Sigma_matrix_inverse,Z_2[:,i])) 
        #Vanilla_Double_robust_predict_2_ideal[:,i]= model_estimate_2[:,i]+np.dot(Sigma_matrix_inverse,Z_2[:,i])*c
        Vanilla_Double_robust_predict_2_sample[:,i]= model_estimate_2[:,i]+np.dot(Sigma_matrix_3_sample_inverse,Z_2[:,i])*c
        Clip_Double_robust_predict_2_sample[:,i]= model_estimate_2[:,i]+np.dot(Modeified_sample_Sigma_matrix_3_inverse,Z_2[:,i])*c
        Lambda_Double_robust_predict_2_sample[:,i]= model_estimate_2[:,i]+np.dot(Sigma_matrix_3_adjustment_sample_lambda_inverse,Z_2[:,i])*c
        Linear_IPW_sample[:,i]=np.dot(Sigma_matrix_3_sample_inverse,Z_2[:,i])*np.array(Y_2.iloc[i,:])
        Linear_IPW_sample_Clip[:,i]=np.dot(Modeified_sample_Sigma_matrix_3_inverse,Z_2[:,i])*np.array(Y_2.iloc[i,:])
        Linear_IPW_sample_Lambda[:,i]=np.dot(Sigma_matrix_3_adjustment_sample_lambda_inverse,Z_2[:,i])*np.array(Y_2.iloc[i,:])
      ...
      if X[1,i]<=0 and X[2,i]<=0:
        ...
        #d_2.append(np.dot(Sigma_matrix_inverse,Z_2[:,i])) 
        #Vanilla_Double_robust_predict_2_ideal[:,i]= model_estimate_2[:,i]+np.dot(Sigma_matrix_inverse,Z_2[:,i])*c
        Vanilla_Double_robust_predict_2_sample[:,i]= model_estimate_2[:,i]+np.dot(Sigma_matrix_4_sample_inverse,Z_2[:,i])*c
        Clip_Double_robust_predict_2_sample[:,i]= model_estimate_2[:,i]+np.dot(Modeified_sample_Sigma_matrix_4_inverse,Z_2[:,i])*c
        Lambda_Double_robust_predict_2_sample[:,i]= model_estimate_2[:,i]+np.dot(Sigma_matrix_4_adjustment_sample_lambda_inverse,Z_2[:,i])*c
        Linear_IPW_sample[:,i]=np.dot(Sigma_matrix_4_sample_inverse,Z_2[:,i])*np.array(Y_2.iloc[i,:])
        Linear_IPW_sample_Clip[:,i]=np.dot(Modeified_sample_Sigma_matrix_4_inverse,Z_2[:,i])*np.array(Y_2.iloc[i,:])
        Linear_IPW_sample_Lambda[:,i]=np.dot(Sigma_matrix_4_adjustment_sample_lambda_inverse,Z_2[:,i])*np.array(Y_2.iloc[i,:])
    ...
    for i in range(m):     
        ...
        if X[1,i]<=0 and X[2,i]>0:
         c=np.array(Y_2.iloc[i,:])-np.array(Y_2_expected[i,:])
         Vanilla_Perfect_Double_robust_predict_ideal_Sigma[:,i]= Y_2_edge_expected[:,i]+np.dot(Sigma_matrix_3_ideal_inverse,Z_2[:,i])*c
         Clip_Perfect_Double_robust_predict_ideal_Sigma[:,i]= Y_2_edge_expected[:,i]+np.dot(Modeified_ideal_Sigma_matrix_3_inverse,Z_2[:,i])*c
         Lambda_Perfect_Double_robust_predict_ideal_Sigma[:,i]= Y_2_edge_expected[:,i]+np.dot(Sigma_matrix_3_adjustment_ideal_lambda_inverse,Z_2[:,i])*c
         #Vanilla_Double_robust_predict_with_sample_Sigma[:,i]= Y_2_edge_expected[:,i]+np.dot(Sigma_matrix_sample_inverse,Z_2[:,i])*c
         Linear_IPW_ideal[:,i]=np.dot(Sigma_matrix_3_ideal_inverse,Z_2[:,i])*np.array(Y_2.iloc[i,:])
         Linear_IPW_ideal_Clip[:,i]=np.dot(Modeified_ideal_Sigma_matrix_3_inverse,Z_2[:,i])*np.array(Y_2.iloc[i,:])
         Linear_IPW_ideal_Lambda[:,i]=np.dot(Sigma_matrix_3_adjustment_ideal_lambda_inverse,Z_2[:,i])*np.array(Y_2.iloc[i,:])
         
         
        if X[1,i]<=0 and X[2,i]<=0:
         c=np.array(Y_2.iloc[i,:])-np.array(Y_2_expected[i,:])
         Vanilla_Perfect_Double_robust_predict_ideal_Sigma[:,i]= Y_2_edge_expected[:,i]+np.dot(Sigma_matrix_4_ideal_inverse,Z_2[:,i])*c
         Clip_Perfect_Double_robust_predict_ideal_Sigma[:,i]= Y_2_edge_expected[:,i]+np.dot(Modeified_ideal_Sigma_matrix_4_inverse,Z_2[:,i])*c
         Lambda_Perfect_Double_robust_predict_ideal_Sigma[:,i]= Y_2_edge_expected[:,i]+np.dot(Sigma_matrix_4_adjustment_ideal_lambda_inverse,Z_2[:,i])*c
         Linear_IPW_ideal[:,i]=np.dot(Sigma_matrix_4_ideal_inverse,Z_2[:,i])*np.array(Y_2.iloc[i,:])
         Linear_IPW_ideal_Clip[:,i]=np.dot(Modeified_ideal_Sigma_matrix_4_inverse,Z_2[:,i])*np.array(Y_2.iloc[i,:])   
         Linear_IPW_ideal_Lambda[:,i]=np.dot(Sigma_matrix_4_adjustment_ideal_lambda_inverse,Z_2[:,i])*np.array(Y_2.iloc[i,:])
...
regret_all_Linear_IPW_Tree= []
validation_all_Linear_IPW_Tree = []
time_all_Linear_IPW_Tree= []
loss_all_Linear_IPW_Tree = []
